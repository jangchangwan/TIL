...
def AtOperator(number):
    return number * 3
  
def PercentOperator(number):
    return number + 5

def SharpOperator(number):
    return number - 7
  
  
def calculation(number, operators):
    for op in operators:
        if op == '@':
            number = AtOperator(number)
        elif op == "%":
            number = PercentOperator(number)
        elif op == '#':
            number = SharpOperator(number)
...
def main():
    ...
    for tc in range(T):
        ...
        num, operators = float(data[0]), data[1:] 
        
        result = calculation(num, operators)
