...
# 테스트 케이스 개수 T 는 입력의 첫 줄에 주어진다. 각 테스트 케이스로는 두 개의 정수 A와 B(A ≠ B)가 공백으로 분리되어 차례로 주어지는데 A는 레지스터의 초기 값을 나타내고 B는 최종 값을 나타낸다. 
...
def BFS(a, b):
    ...
    while front != rear:
        ...
        # L 명령어
        num3 = ( 10 * number + ( number // 1000 )) % 10000
        if not visited[num3]:
            queue.append((num3, path + 'L'))
            visited[num3] = True
            rear += 1

        # R 명령어
        num4 = ( number // 10 + ( number % 10 ) * 1000 ) % 10000
        if not visited[num4]:
            queue.append((num4, path + "R"))
            visited[num4] = True
            rear += 1
    return -1
