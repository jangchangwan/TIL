...
# (단 도로는 방향이 없으며 웜홀은 방향이 있다.) 웜홀은 시작 위치에서 도착 위치로 가는 하나의 경로인데, 특이하게도 도착을 하게 되면 시작을 하였을 때보다 시간이 뒤로 가게 된다. 
...
# 벨만포드 알고리즘
# 음수가 포함된 최적값을 찾기위해서는 다익스트라 사용 불가능
def bellman_ford(start):
    dis = [INF] * (N + 1)
    dis[start]=0
    for i in range(N):
        for d in distance:
            cur, next_node, cost = d 
...
for tc in range(TC):
    ...
    for _ in range(M):
        ...
        distance.append((S, E, T))
        distance.append((E, S, T))
